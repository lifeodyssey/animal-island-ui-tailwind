 ...
 *  - 自定义组件（如本库 Select/Radio/Checkbox）：onChange 直接传值，原样返回
 */
...
const prefixCls = 'animal-form-item';
...
export const FormItem: React.FC<FormItemProps> = (props) => {
    ...
    const childProps: Record<string, unknown> = childIsElement
    ...
    const renderChildren =
        fieldKey && childIsElement ? React.cloneElement(children, childProps) : children;
    ...
    const buildGridStyle = (
        col: { span?: number; offset?: number } | undefined,
        startCol = 1
    ): React.CSSProperties => {
        if (!col) return {};
    ...
    const rootCls = cn(
        ...
        `${prefixCls}-${layout}`,
        `${prefixCls}-${size}`,
        computedStatus === 'error' && `${prefixCls}-has-error`,
        computedStatus === 'warning' && `${prefixCls}-has-warning`,
        computedStatus === 'success' && `${prefixCls}-has-success`,
        computedStatus === 'validating' && `${prefixCls}-is-validating`,
        showRequiredMark && `${prefixCls}-required`,
        className
    );
    ...
    const labelNode =
        label !== undefined ? (
            <label
                ...
                className={cn(`${prefixCls}-label`, {
                    [`${prefixCls}-label-required`]: showRequiredMark,
                    [`${prefixCls}-label-colon`]: showColon && label !== '',
                })}
    ...
    const helpNode =
        showHelp !== undefined ? (
            <div
                className={cn(`${prefixCls}-explain`, {
                    [`${prefixCls}-explain-error`]: computedStatus === 'error',
                })}
            >
    ...
    return (
        <div className={rootCls} data-field-name={fieldKey ?? undefined}>
            {labelNode}
            <div className={`${prefixCls}-control`} style={wrapperColStyle}>
                <div className={`${prefixCls}-control-input`}>{renderChildren}</div>
                {helpNode}
            </div>
        </div>
    );
