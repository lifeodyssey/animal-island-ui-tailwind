...
export interface TitleProps {
    ...
    /** 配色，与 Card 同名色板 */
    color?: TitleColor;
    /** 样式变体: ribbon 飘带 / layer 双层纸（默认） / tab 折角便签 */
...
// Shared color CVA — color variant adds the same modifier class for all three shapes.
...
const Ribbon: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span
        className={cn('animal-title-ribbon', colorVariants({ color }))}
...
const Layer: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span
        className={cn('animal-title-layer', colorVariants({ color }))}
        style={{ fontSize: `${fontSize}px` }}
    >
...
const Tab: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span
        className={cn('animal-title-tab', colorVariants({ color }))}
        style={{ fontSize: `${fontSize}px` }}
    >
