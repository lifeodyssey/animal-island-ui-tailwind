...
const Ribbon: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span className={cn(ribbonVariants({ color }))} style={{ fontSize: `${fontSize}px` }}>
...
const Layer: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span className={cn(layerVariants({ color }))} style={{ fontSize: `${fontSize}px` }}>
...
const Tab: React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }> = ({
    ...
    <span className={cn(tabVariants({ color }))} style={{ fontSize: `${fontSize}px` }}>
        <span className="animal-title-tab-text">{children}</span>
    </span>
);

const VARIANT_MAP: Record<TitleVariant, React.FC<{ children: React.ReactNode; fontSize: number; color: TitleColor }>> = {
    ribbon: Ribbon,
    layer: Layer,
    tab: Tab,
};
...
export const Title: React.FC<TitleProps> = ({
    ...
    const VariantCmp = VARIANT_MAP[variant];
    return (
        <span className={cn('animal-title', className)} style={style}>
            <VariantCmp fontSize={SIZE_MAP[size]} color={color}>
                {children}
            </VariantCmp>
        </span>
    );
