...
export const Time = React.forwardRef<HTMLDivElement, TimeProps>(({ className, type = 'game', ...rest }, ref) => {
    ...
    if (type === 'game') {
        return (
            <div ref={ref} className={cn('animal-time-game', className)} style={{ padding: 50 }} {...rest}>
        ...
        <Cursor>
            <div ref={ref} className={cn('animal-time', className)} {...rest}>
                <div className="animal-time-date">
                    <span className="animal-time-weekday">{weekdays[currentTime.getDay()]}</span>
                    <span className="animal-time-monthday">{months[currentTime.getMonth()]} {currentTime.getDate()}</span>
                </div>
