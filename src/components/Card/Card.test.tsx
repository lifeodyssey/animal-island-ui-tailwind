...
describe('Card', () => {
    ...
    it('color=default 显式不应用任何 animal-card-${color} 类', () => {
        ...
        const root = container.firstChild as HTMLElement;
        expect(root).not.toHaveClass('animal-card-app-pink');
        expect(root).not.toHaveClass('animal-card-purple');
    });

    it('pattern=none 显式不应用任何 animal-card-pattern-${pattern} 类', () => {
        const { container } = render(<Card pattern="none">x</Card>);
        const root = container.firstChild as HTMLElement;
        expect(root).not.toHaveClass('animal-card-pattern-app-pink');
        expect(root).not.toHaveClass('animal-card-pattern-default');
    });
