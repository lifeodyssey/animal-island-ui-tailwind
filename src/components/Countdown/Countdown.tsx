...
const formatRemaining = (remaining: number, format: string) => {
    ...
    return format.replace(/DD/g, pad(days)).replace(/HH/g, pad(hours)).replace(/mm/g, pad(minutes)).replace(/ss/g, pad(seconds));
};

export const Countdown: React.FC<CountdownProps> = ({
    value, format = 'HH:mm:ss', prefix, size = 'middle', variant = 'default',
    onChange, onFinish, className, ...rest
}) => {
    ...
    return (
        <div
            className={cn(
                'animal-countdown',
                size === 'small' && 'animal-countdown-small',
                size === 'large' && 'animal-countdown-large',
                variant === 'island' && 'animal-countdown-island',
                className
            )}
            role="timer"
            aria-live="off"
