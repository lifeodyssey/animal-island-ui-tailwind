...
export const Drawer: React.FC<DrawerProps> = ({
    ...
    useEffect(() => {
        ...
        const pushed: Array<{ el: HTMLElement; transform: string; filter: string; borderRadius: string; overflow: string; transition: string; }> = [];
        ...
        candidates.forEach((el) => {
            pushed.push({ el, transform: el.style.transform, filter: el.style.filter, borderRadius: el.style.borderRadius, overflow: el.style.overflow, transition: el.style.transition });
            el.style.transition = 'transform 0.36s cubic-bezier(0.2, 0, 0.2, 1), filter 0.36s ease, border-radius 0.36s ease';
        });
        const rafId = requestAnimationFrame(() => {
            pushed.forEach(({ el }) => { el.style.transform = 'scale(0.94)'; el.style.filter = 'blur(1px)'; el.style.borderRadius = '14px'; el.style.overflow = 'hidden'; });
        });
        return () => {
            cancelAnimationFrame(rafId);
            if (restoreTransitionRafRef.current !== null) { cancelAnimationFrame(restoreTransitionRafRef.current); restoreTransitionRafRef.current = null; }
            pushed.forEach(({ el, transform, filter, borderRadius, overflow }) => { el.style.transform = transform; el.style.filter = filter; el.style.borderRadius = borderRadius; el.style.overflow = overflow; });
            restoreTransitionRafRef.current = requestAnimationFrame(() => {
                pushed.forEach(({ el, transition }) => { el.style.transition = transition; });
                restoreTransitionRafRef.current = null;
            });
        };
    }, [open, pushBackground]);

    const handleMaskClick = useCallback(() => { if (maskClosable) onClose?.(); }, [maskClosable, onClose]);
                    ...
                    >
                        {title && (
                            <div className="animal-drawer-header">
                                <div className="animal-drawer-title" id={titleId}>{title}</div>
                                <button type="button" className="animal-drawer-close" onClick={onClose} aria-label="关闭">×</button>
                            </div>
                        )}
