...
/**
 * Drawer with push-background depth effect.
 *
 * The panel always stays in the DOM:
 * - Closed state: placement class sets transform: translateX/Y(...)
 * - Open state: panelOpen overrides with transform: none — CSS transition plays forward/reverse automatically.
 * - aria-hidden + inert exclude the drawer from a11y tree and Tab order when closed.
 */
export const Drawer: React.FC<DrawerProps> = ({
    ...
    // Push-background depth effect: scale/blur non-fixed body children.
    // Values match index.html "sink depth drawer" reference:
    //   transform: scale(0.94), filter: blur(1px), border-radius: 14px, overflow: hidden
    useEffect(() => {
        ...
        const candidates = Array.from(document.body.children).filter((el): el is HTMLElement => {
            ...
            // Skip our own portal (Cursor wrapper is non-fixed but its children are fixed; applying
            // transform here would make it a containing block for fixed descendants, breaking positioning).
