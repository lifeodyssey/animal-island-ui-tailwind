...
describe('Drawer', () => {
    ...
    it('点击抽屉内容不冒泡触发 onClose', async () => {
        const user = userEvent.setup();
        const onClose = vi.fn();
        render(<Drawer open onClose={onClose}><p>inside</p></Drawer>);
    ...
    it('默认 pushBackground 下沉 body 非固定子元素', async () => {
        ...
        render(<Drawer open><p>drawer</p></Drawer>);
        await waitFor(() => { expect(content.style.transform).toBe('scale(0.94)'); });
    ...
    it('pushBackground=false 不下沉背景', () => {
        ...
        content.textContent = 'page content';
        document.body.appendChild(content);
        render(<Drawer open pushBackground={false}><p>drawer</p></Drawer>);
    ...
    it('关闭后恢复背景元素原始样式', async () => {
        ...
        content.textContent = 'page content';
        document.body.appendChild(content);
        const { rerender } = render(<Drawer open><p>drawer</p></Drawer>);
        await waitFor(() => { expect(content.style.transform).toBe('scale(0.94)'); });
        rerender(<Drawer open={false}><p>drawer</p></Drawer>);
        expect(content.style.transform).toBe('');
    ...
    it('footer 传入时渲染', () => {
        render(<Drawer open footer={<button>ok</button>}>body</Drawer>);
    ...
    describe('a11y', () => {
        it('aria-labelledby 关联 title', () => {
            render(<Drawer open title="嗨标题"><p>嗨内容</p></Drawer>);
            const dialog = screen.getByRole('dialog');
        ...
        it('关闭按钮 aria-label="关闭" 且触发 onClose', async () => {
            const user = userEvent.setup();
            const onClose = vi.fn();
            render(<Drawer open title="t" onClose={onClose}>body</Drawer>);
        ...
        it('Tab 焦点陷阱：末尾元素 Tab 回到第一个', async () => {
            ...
            await waitFor(() => { expect(screen.getByLabelText('关闭')).toHaveFocus(); });
            await user.tab();
