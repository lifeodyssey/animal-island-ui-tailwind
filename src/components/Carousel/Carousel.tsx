...
export const Carousel: React.FC<CarouselProps> = ({
    children, activeIndex, defaultActiveIndex = 0, onChange,
    autoplay = false, interval = 3_000, loop = true, showArrows = true, showDots = true,
    pauseOnHover = true, className, onKeyDown, onMouseEnter, onMouseLeave, onFocus, onBlur,
    'aria-label': ariaLabel = '轮播图', ...rest
}) => {
    ...
    return (
        <section
            ...
            onMouseEnter={(event) => { if (pauseOnHover) setHoverPaused(true); onMouseEnter?.(event); }}
            onMouseLeave={(event) => { setHoverPaused(false); onMouseLeave?.(event); }}
            onFocus={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setFocusPaused(true); onFocus?.(event); }}
            onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setFocusPaused(false); onBlur?.(event); }}
            {...rest}
        >
            {hasControls && autoplay && (
                <button
                    ...
                    onClick={() => {
                        if (effectivePaused) { setHoverPaused(false); setFocusPaused(false); setRotationPaused(false); }
                        else { setRotationPaused(true); }
                    }}
                >
            ...
            {hasControls && showArrows && (
                <>
                    <button type="button" className="animal-carousel-arrow animal-carousel-arrow-previous" aria-label="上一张" disabled={!loop && currentIndex === 0} onClick={() => goTo(currentIndex - 1)} />
                    <button type="button" className="animal-carousel-arrow animal-carousel-arrow-next" aria-label="下一张" disabled={!loop && currentIndex === lastIndex} onClick={() => goTo(currentIndex + 1)} />
                </>
            )}
