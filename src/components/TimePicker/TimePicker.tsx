...
export const TimePicker: React.FC<TimePickerProps> = ({
    ...
    const triggerCls = [
        'animal-timepicker-trigger',
        `animal-timepicker-trigger-${size}`,
        status && `animal-timepicker-trigger-${status}`,
        open && 'animal-timepicker-trigger-open',
    ]
        .filter(Boolean)
        .join(' ');
    ...
    return (
        <div
            ref={wrapperRef}
            className={['animal-timepicker-wrapper', disabled && 'animal-timepicker-wrapper-disabled', className].filter(Boolean).join(' ')}
            style={style}
        ...
        >
            ...
            >
                <span className={currentValue ? 'animal-timepicker-value' : 'animal-timepicker-placeholder'}>
                    {open && pending ? formatTime(pending, format) : parsed ? formatTime(parsed, format) : placeholder}
                </span>
                {allowClear && currentValue && !disabled && (
                    <button
                        type="button"
                        className="animal-timepicker-clear"
                        aria-label="清除时间"
                        onClick={handleClear}
                ...
                <span className="animal-timepicker-clock-icon" aria-hidden>
            ...
            {open && (
                <div
                    ...
                    role="dialog"
                    aria-label="选择时间"
                    className={`animal-timepicker-panel ${!hasSeconds(format) ? 'animal-timepicker-panel-no-seconds' : ''} ${
                        closing ? 'animal-timepicker-panel-closing' : mounted ? 'animal-timepicker-panel-visible' : ''
                    }`}
                    style={panelStyle}
                >
                    <div className="animal-timepicker-columns">
                        <div className="animal-timepicker-column">
                            <div className="animal-timepicker-column-header">时</div>
                            <div ref={hourListRef} className="animal-timepicker-column-list">
                                {hours.map((h) => (
                                    <button
                                        key={h}
                                        type="button"
                                        className={`animal-timepicker-option ${base.h === h ? 'animal-timepicker-option-selected' : ''}`}
                                        aria-label={`${h} 时`}
                        ...
                        <div className="animal-timepicker-column">
                            <div className="animal-timepicker-column-header">分</div>
                            <div ref={minuteListRef} className="animal-timepicker-column-list">
                                {minutes.map((m) => (
                                    <button
                                        key={m}
                                        type="button"
                                        className={`animal-timepicker-option ${base.m === m ? 'animal-timepicker-option-selected' : ''}`}
                                        aria-label={`${m} 分`}
                        ...
                        {seconds.length > 0 && (
                            <div className="animal-timepicker-column">
                                <div className="animal-timepicker-column-header">秒</div>
                                <div ref={secondListRef} className="animal-timepicker-column-list">
                                    {seconds.map((s) => (
                                        <button
                                            key={s}
                                            type="button"
                                            className={`animal-timepicker-option ${base.s === s ? 'animal-timepicker-option-selected' : ''}`}
                                            aria-label={`${s} 秒`}
                    ...
                    <div className="animal-timepicker-footer">
                        <button
                            type="button"
                            className="animal-timepicker-now-btn"
                            onClick={setNow}
                            onMouseDown={(e) => e.preventDefault()}
                        >
                            此刻
                        ...
                        <button
                            type="button"
                            className="animal-timepicker-confirm-btn"
                            onClick={confirmTime}
                            onMouseDown={(e) => e.preventDefault()}
                        >
                            确定
