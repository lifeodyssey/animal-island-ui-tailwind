...
const triggerVariants = cva('animal-time-picker-trigger', {
    variants: {
        size: {
            small: 'animal-time-picker-trigger-small',
            middle: 'animal-time-picker-trigger-middle',
            large: 'animal-time-picker-trigger-large',
        },
        status: {
            error: 'animal-time-picker-trigger-error',
            warning: 'animal-time-picker-trigger-warning',
        },
        open: { true: 'animal-time-picker-trigger-open' },
    },
...
export const TimePicker: React.FC<TimePickerProps> = ({
    ...
    const panelId = `animal-time-picker-${idSuffix}-panel`;
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        if (key === 'Escape') { e.preventDefault(); closePanel(); triggerRef.current?.focus(); }
        else if (key === 'Enter') { e.preventDefault(); confirmTime(); }
    };
                            ...
                            <div ref={hourListRef} className="animal-time-picker-column-list">
                                {hours.map((h) => (
                                    <button
                                        key={h}
                                        type="button"
                                        className={cn('animal-time-picker-option', base.h === h && 'animal-time-picker-option-selected')}
                                        aria-label={`${h} 时`}
                            ...
                            <div ref={minuteListRef} className="animal-time-picker-column-list">
                                {minutes.map((m) => (
                                    <button
                                        key={m}
                                        type="button"
                                        className={cn('animal-time-picker-option', base.m === m && 'animal-time-picker-option-selected')}
                                        aria-label={`${m} 分`}
                                ...
                                <div ref={secondListRef} className="animal-time-picker-column-list">
                                    {seconds.map((s) => (
                                        <button
                                            key={s}
                                            type="button"
                                            className={cn('animal-time-picker-option', base.s === s && 'animal-time-picker-option-selected')}
                                            aria-label={`${s} 秒`}
