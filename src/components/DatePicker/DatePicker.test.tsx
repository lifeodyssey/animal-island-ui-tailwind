...
describe('DatePicker', () => {
    ...
    describe('disabled / clear', () => {
        it('disabled 禁用触发区且不可展开', async () => {
            ...
            expect(trigger.parentElement).toHaveClass('animal-date-picker-disabled');
        ...
        it('disabledDate 禁用周末日期且不可选中', async () => {
            ...
            expect(weekend).toHaveClass('animal-date-picker-day-disabled');
    ...
    describe('年 / 月 / 日视图切换', () => {
        ...
        it('单日期模式圈出今天', async () => {
            ...
            expect(todayCell).toHaveClass('animal-date-picker-day-today');
        });
    });
    ...
    describe('范围选择模式', () => {
        ...
        it('选中范围端点与区间应用高亮类', async () => {
            ...
            render(<DatePicker range defaultValue={['2026-08-10', '2026-08-12']} />);
            await user.click(screen.getByRole('combobox'));
            expect(screen.getByRole('button', { name: '2026年8月10日' })).toHaveClass('animal-date-picker-day-range-start');
            expect(screen.getByRole('button', { name: '2026年8月12日' })).toHaveClass('animal-date-picker-day-range-end');
            expect(screen.getByRole('button', { name: '2026年8月11日' })).toHaveClass('animal-date-picker-day-in-range');
        });
        ...
        it('开始新选择时旧范围高亮让位', async () => {
            ...
            await user.click(screen.getByRole('combobox'));
            await user.click(screen.getByRole('button', { name: '2026年8月15日' }));
            expect(screen.getByRole('button', { name: '2026年8月12日' })).not.toHaveClass('animal-date-picker-day-range-end');
            expect(screen.getByRole('button', { name: '2026年8月15日' })).toHaveClass('animal-date-picker-day-range-start');
        });
        ...
        it('正向悬停预览：开始日期到悬停日期区间高亮', async () => {
            ...
            expect(screen.getByRole('button', { name: '2026年8月10日' })).toHaveClass('animal-date-picker-day-range-start');
            expect(screen.getByRole('button', { name: '2026年8月15日' })).toHaveClass('animal-date-picker-day-range-end');
            expect(screen.getByRole('button', { name: '2026年8月12日' })).toHaveClass('animal-date-picker-day-in-range');
        });
        ...
        it('范围模式不圈出今天', async () => {
            ...
            expect(todayCell).not.toHaveClass('animal-date-picker-day-today');
        });
    });
});
