...
const DoubleChevronLeft = (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
        ...
        <path d="M13 4L9 8l4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
);
...
export const DatePicker: React.FC<DatePickerProps> = ({
    ...
    return (
        <div
            ref={wrapperRef}
            className={cn('animal-date-picker-wrapper', disabled && 'animal-date-picker-wrapper-disabled', className)}
            style={style}
                ...
                >
                    {range ? (
                        ...
                        <>
                            ...
                            {mode === 'month' && (
                                <div className="animal-date-picker-grid3x4">
                                    {MONTHS.map((label, i) => {
                                        const activeDate = pendingDate ?? selectedDate;
                                        const selected =
                                            !!activeDate &&
                                        ...
                                        return (
                                            <button
                                                key={label}
                                                type="button"
                                                className={cn('animal-date-picker-month-cell', selected && 'animal-date-picker-month-cell-selected')}
                                                aria-label={`${i + 1}月`}
                                                onClick={() => {
                                                    if (picker === 'month' && !range) {
                                                        // 月份选择模式：点击即设为待选月份
                                    ...
                                    {yearCells.map((y) => {
                                        ...
                                        return (
                                            <button
                                                key={y}
                                                type="button"
                                                className={cn('animal-date-picker-year-cell', selected && 'animal-date-picker-year-cell-selected')}
                                                aria-label={`${y}年`}
                                                onClick={() => {
