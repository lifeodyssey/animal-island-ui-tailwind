...
const DoubleChevronLeft = (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden>
        <path d="M8 3L4.5 7l3.5 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M11 3L7.5 7l3.5 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
);

const DoubleChevronRight = (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden>
        <path d="M6 3L9.5 7 6 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M3 3L6.5 7 3 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
);
...
export const DatePicker: React.FC<DatePickerProps> = ({
        ...
        return (
            <>
                <div className="animal-date-picker-week-row">
                    {WEEKDAYS.map((w) => (
                        <div key={w} className="animal-date-picker-week-cell">
                            {w}
                        </div>
                    ))}
                </div>
                <div className="animal-date-picker-grid">
                    {buildCells(vDate).map((cell) => {
                        ...
                        const cls = cn(
                            'animal-date-picker-day',
                            outside && 'animal-date-picker-day-outside',
                            !range && isToday && 'animal-date-picker-day-today',
                            selected && 'animal-date-picker-day-selected',
                            rangeStartCell && 'animal-date-picker-day-range-start',
                            rangeEndCell && 'animal-date-picker-day-range-end',
                            inRange && 'animal-date-picker-day-in-range',
                            disabledCell && 'animal-date-picker-day-disabled'
                        );
                        return (
                            <button
                                key={cell.getTime()}
    ...
    const triggerCls = cn(
        'animal-date-picker-trigger',
        `animal-date-picker-trigger-${size}`,
        status && `animal-date-picker-trigger-${status}`,
        open && 'animal-date-picker-trigger-open'
    );
    ...
    return (
        <div
            ...
            className={cn(
                'animal-date-picker',
                disabled && 'animal-date-picker-disabled',
                className
            )}
            style={style}
                    ...
                    {range ? (
                        <>
                                            ...
                                            <div className="animal-date-picker-header">
                                                <div className="animal-date-picker-header-group">
                                                    {idx === 0 && (
                                                        <button
                                                            type="button"
                                                            className="animal-date-picker-nav-btn"
                                                            aria-label="上一年"
                            ...
                            <div className="animal-date-picker-footer">
                                <button
                                    type="button"
                                    className="animal-date-picker-confirm-btn"
                                    onClick={confirmTime}
                                    onMouseDown={(e) => e.preventDefault()}
                                >
                                    确定
                        ...
                        <>
                            <div className="animal-date-picker-header">
                                ...
                                {mode === 'date' && (
                                    <button
                                        type="button"
                                        className="animal-date-picker-label-btn"
                                    ...
                                    {MONTHS.map((label, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                type="button"
                                                className={cn(
                                                    'animal-date-picker-month-cell',
                                                    isSelected && 'animal-date-picker-month-cell-selected'
                                                )}
                                                aria-label={`${i + 1}月`}
                                                onClick={() => {
                                                    const next = new Date(year, i, 1);
                                                    setViewDate(next);
                                                    setFocusedDate(next);
                                                    if (picker === 'month') {
                                                        setPendingDate(next);
                                    ...
                                    {yearCells.map((y) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                type="button"
                                                className={cn(
                                                    'animal-date-picker-year-cell',
                                                    isSelected && 'animal-date-picker-year-cell-selected'
                                                )}
                                                aria-label={`${y}年`}
                                                onClick={() => {
                                                    const next = new Date(y, viewDate.getMonth(), 1);
                                                    setViewDate(next);
                                                    setFocusedDate(next);
                            ...
                            {showToday && mode === 'date' && !range && (
                                <div className="animal-date-picker-footer">
                                    <button
                                        type="button"
                                        className="animal-date-picker-today-btn"
                                        onClick={handleToday}
                                        onMouseDown={(e) => e.preventDefault()}
                                    >
                                        今天
