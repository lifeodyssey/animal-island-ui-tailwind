// Notification imperative API + internal store + container.
// Usage: Notification.success({ message: '...', description: '...' })
// Maintains a global items array; mounts a React root to body on first call.
// Uses useSyncExternalStore for React to subscribe to store changes.
        ...
        <>
            {POSITION_GROUPS.map((position) => {
                ...
                return (
                    <div
                        key={position}
                        className={cn(`animal-notification-pos-${position}`)}
...
const destroy = (key?: string): void => {
    ...
    listeners.forEach((l) => l());
    // Align with × / duration expiry contract: onClose fires on any close path.
    // Callers set dismissed/abort flags in onClose; without this, destroy() misses those signals.
    removed.forEach((it) => it.onClose?.());
};

/**
 * Imperative Notification, antd-style static methods.
 *
 * - `Notification.open(config)` / `Notification.success(config)` / `.info(config)` / `.warning(config)` / `.error(config)`
 * - `config` may be a string (message only) or a full object
 * - `Notification.destroy()` closes all; `Notification.destroy(key)` closes the specified key
 * - Default position: `top` (centered at top)
 * - Default `duration: 4.5` seconds; pass `0` to disable auto-close
 * - Explicit `key`: re-calling with same key updates the existing notification instead of adding a new one
 */
