...
describe('Notification', () => {
    ...
    describe('静态方法挂载与渲染', () => {
        ...
        it('不同 type 应用对应 animal-notification-{type} class', async () => {
            act(() => {
            ...
            await waitFor(() => {
                ...
                expect(items[0].className).toContain('animal-notification-success');
                expect(items[1].className).toContain('animal-notification-error');
                expect(items[2].className).toContain('animal-notification-warning');
                expect(items[3].className).toContain('animal-notification-info');
            });
        });
    });
    ...
    describe('位置分组', () => {
        it('position=top 默认时通知挂到 top 组', async () => {
            ...
            await waitForContainer();
            await waitFor(() => {
                const topGroup = document.querySelector('.animal-notification-pos-top');
        ...
        it('position=topRight 走 topRight 组', async () => {
            ...
            await waitForContainer();
            await waitFor(() => {
                const group = document.querySelector('.animal-notification-pos-topRight');
        ...
        it('position=bottom 走 bottom 组，placement=bottom', async () => {
            ...
            await waitForContainer();
            await waitFor(() => {
                const group = document.querySelector('.animal-notification-pos-bottom');
                expect(group).not.toBeNull();
    ...
    describe('关闭行为', () => {
        ...
        it('destroy(key) 同步触发该项 onClose，其它 key 不触发', async () => {
        ...
        it('upload 场景：destroy 后同 key 后续 open 不再创建', async () => {
    ...
    describe('race: closeIcon onClick 立即置 dismissed', () => {
        it('点 closeIcon 同步置 dismissed，避免退场动画期被同 key 复活', async () => {
            const user = userEvent.setup();
