...
describe('Notification', () => {
    ...
    describe('静态方法挂载与渲染', () => {
        ...
        it('字符串简写会作为 message 渲染', async () => {
            act(() => { Notification.success('简写消息'); });
            await waitForContainer();
            await waitFor(() => { expect(screen.getByText('简写消息')).toBeInTheDocument(); });
        });

        it('对象 config 渲染 message + description', async () => {
            act(() => { Notification.info({ message: '标题', description: '详细描述内容' }); });
            await waitForContainer();
            await waitFor(() => {
    ...
    describe('位置分组', () => {
        it('position=top 默认时通知挂到 top 组', async () => {
            act(() => { Notification.info({ message: 'top' }); });
            await waitForContainer();
            await waitFor(() => {
        ...
        it('position=topRight 走 topRight 组', async () => {
            act(() => { Notification.info({ message: 'right', position: 'topRight' }); });
            await waitForContainer();
            await waitFor(() => {
        ...
        it('position=bottom 走 bottom 组,placement=bottom', async () => {
            act(() => { Notification.info({ message: 'b', position: 'bottom' }); });
            await waitForContainer();
            await waitFor(() => {
    ...
    describe('关闭行为', () => {
        it('点击关闭按钮触发退场后从 DOM 移除', async () => {
            ...
            act(() => { Notification.info({ message: 'close me' }); });
            await waitForContainer();
        ...
        it('duration 较短时自动关闭并触发 onClose', async () => {
            const onClose = vi.fn();
            act(() => { Notification.info({ message: 'auto', duration: 0.5, onClose }); });
            await waitForContainer();
        ...
        it('destroy() 关闭全部', async () => {
            act(() => { Notification.info('a'); Notification.success('b'); });
            await waitForContainer();
            await waitFor(() => {
                expect(screen.getByText('a')).toBeInTheDocument();
                expect(screen.getByText('b')).toBeInTheDocument();
            });
            act(() => { notificationDestroy(); });
            await waitForGone('a');
            await waitForGone('b');
        });
        ...
        it('upload 场景:destroy 后同 key 后续 open 也不再创建', async () => {
            ...
            let dismissed = false;
            const onCloseMock = vi.fn(() => { dismissed = true; });
            const openN = (msg: string) => {
                if (dismissed) return;
                Notification.info({ message: msg, key: uploadKey, duration: 0, onClose: onCloseMock });
            };
            act(() => { openN('上传中 0%'); });
            await waitForContainer();
            await waitFor(() => { expect(screen.getByText('上传中 0%')).toBeInTheDocument(); });
            act(() => { notificationDestroy(); });
            expect(onCloseMock).toHaveBeenCalledTimes(1);
            expect(dismissed).toBe(true);
            await waitForGone('上传中 0%');
            act(() => { openN('上传中 50%'); openN('上传完成 100%'); });
            await wait(400);
    ...
    describe('onClick 与可点击态', () => {
        it('配置 onClick 后,通知本体可点击触发回调', async () => {
            const user = userEvent.setup();
            const onClick = vi.fn();
            act(() => { Notification.info({ message: 'click me', onClick }); });
            await waitForContainer();
        ...
        it('onClick 设置后获得 role=button 与 tabIndex=0', async () => {
            act(() => { Notification.info({ message: 'kb', onClick: () => {} }); });
            await waitForContainer();
    ...
    describe('key 更新', () => {
        it('同 key 二次 open 走更新分支(仍只 1 条)', async () => {
            act(() => { Notification.info({ message: 'first', key: 'same' }); });
            await waitForContainer();
            await waitFor(() => { expect(screen.getByText('first')).toBeInTheDocument(); });
            act(() => { Notification.info({ message: 'second', key: 'same' }); });
            await waitFor(() => {
                const root = getContainer()!;
                const items = root.querySelectorAll('.animal-notification');
    ...
    describe('btn slot', () => {
        it('配置 btn 后渲染自定义操作按钮', async () => {
            ...
            await waitForContainer();
            await waitFor(() => { expect(screen.getByTestId('custom-btn')).toBeInTheDocument(); });
        });
    });

    describe('race: closeIcon dismiss', () => {
        it('closeIcon onClick 立即置 dismissed,避免退场动画期被同 key 复活', async () => {
            const user = userEvent.setup();
            ...
            act(() => { openN('race-0'); });
            await waitForContainer();
            await waitFor(() => { expect(screen.getByText('race-0')).toBeInTheDocument(); });
            const closeBtn = screen.getByLabelText('close');
            const closeIcon = within(closeBtn).getByText('×');
            await user.click(closeIcon);
            act(() => { openN('race-50'); openN('race-100'); });
