...
export const NotificationView: React.FC<NotificationViewProps> = ({ item, onRemove }) => {
    ...
    useEffect(() => {
        ...
        closeTimerRef.current = window.setTimeout(() => triggerClose(), item.duration * 1000);
        return () => {
    ...
    useEffect(() => {
        ...
        return () => {
            if (removeTimerRef.current !== null) { window.clearTimeout(removeTimerRef.current); removeTimerRef.current = null; }
        };
    ...
    return (
        ...
        >
            <div className="animal-notification-icon" aria-hidden>{iconNode}</div>
            ...
            <button type="button" className="animal-notification-close" aria-label="close" onClick={handleCloseClick}>
