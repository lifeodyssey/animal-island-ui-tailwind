...
describe('Progress', () => {
    ...
    describe('info text', () => {
        ...
        it('infoPosition=right places percent after the track', () => {
            ...
            expect(container.querySelector('.animal-progress-row')).toBeInTheDocument();
        });
    });
    ...
    describe('size', () => {
        it('size=small applies size-small class to track', () => {
            ...
            expect(container.querySelector('.animal-progress-size-small')).toBeInTheDocument();
        });

        it('size=middle applies size-middle class to track (default)', () => {
            const { container } = render(<Progress percent={50} />);
            expect(container.querySelector('.animal-progress-size-middle')).toBeInTheDocument();
        });

        it('size=large applies size-large class to track', () => {
            const { container } = render(<Progress percent={50} size="large" />);
            expect(container.querySelector('.animal-progress-size-large')).toBeInTheDocument();
        });
    });

    describe('animation', () => {
        it('duration=0 applies no-transition class to fill', () => {
        ...
        it('default fill has stripe animation class', () => {
            const { container } = render(<Progress percent={50} />);
            const fillEl = container.querySelector('.animal-progress-fill') as HTMLElement;
            expect(fillEl).toBeInTheDocument();
    ...
    describe('inside info fallback for low percent', () => {
        it('shows percent outside fill when percent < 18% to keep white text readable', () => {
        ...
        it('shows percent inside fill when percent >= 18%', () => {
