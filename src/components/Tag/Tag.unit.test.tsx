...
describe('Tag', () => {
    describe('rendering', () => {
        ...
        it('支持 className 与 style', () => {
            const { container } = render(
                <Tag className="x" style={{ marginLeft: 4 }}>t</Tag>
            );
            const root = container.firstChild as HTMLElement;
    ...
    describe('color', () => {
        it('color=default 不应用任何 color 类', () => {
            ...
            expect((container.firstChild as HTMLElement).className).not.toMatch(/animal-tag-\w+-solid/);
        });

        it('color=app-pink + solid 应用 animal-tag-app-pink-solid 类', () => {
            const { container } = render(<Tag color="app-pink" variant="solid">x</Tag>);
            expect(container.firstChild).toHaveClass('animal-tag-app-pink-solid');
        });

        it('color=purple + outlined 应用 animal-tag-purple-outlined 类', () => {
            const { container } = render(<Tag color="purple" variant="outlined">x</Tag>);
            expect(container.firstChild).toHaveClass('animal-tag-purple-outlined');
        });

        it('color=app-blue + dashed 应用 animal-tag-app-blue-dashed 类', () => {
            const { container } = render(<Tag color="app-blue" variant="dashed">x</Tag>);
            expect(container.firstChild).toHaveClass('animal-tag-app-blue-dashed');
        });
    });
    ...
    describe('closable', () => {
        ...
        it('disabled 状态下点击关闭按钮不触发 onClose', () => {
            const onClose = vi.fn();
            render(<Tag closable disabled onClose={onClose}>x</Tag>);
    ...
    describe('clickable', () => {
        ...
        it('disabled 状态下不响应 onClick', () => {
            const onClick = vi.fn();
            const { container } = render(<Tag disabled onClick={onClick}>x</Tag>);
            const root = container.firstChild as HTMLElement;
    ...
    describe('event isolation', () => {
        it('点击关闭按钮不会冒泡触发 onClick', () => {
            const onClick = vi.fn();
            const onClose = vi.fn();
            render(<Tag closable onClose={onClose} onClick={onClick}>x</Tag>);
            fireEvent.click(screen.getByRole('button', { name: 'close' }));
            expect(onClose).toHaveBeenCalledTimes(1);
            expect(onClick).not.toHaveBeenCalled();
        });
